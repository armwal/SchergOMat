...
namespace Chummer
{
	public class CommonFunctions
	{
		...
		public ITreeNode FindNode(string strGuid, ITreeView treTree)
		{
            ITreeNode objFound = displayFactory.CreateTreeNode();

            foreach (ITreeNode objNode in treTree.Nodes)
			{
		...
		public ITreeNode FindNode(string strGuid, ITreeNode objNode)
		{
            ITreeNode objFound = displayFactory.CreateTreeNode();
			foreach (ITreeNode objChild in objNode.Nodes)
			{
		...
		public void DeleteGear(Gear objGear, ITreeView treWeapons, ImprovementManager objImprovementManager)
		{
			...
			if (objGear.WeaponID != Guid.Empty.ToString())
			{
                ...
                ITreeNode objRemoveNode = displayFactory.CreateTreeNode();
				foreach (ITreeNode objWeaponNode in treWeapons.Nodes[0].Nodes)
				{
		...
		public void DeleteVehicleGear(Gear objGear, ITreeView treVehicles, Vehicle objVehicle)
		{
			// Remove any children the Gear may have.
			foreach (Gear objChild in objGear.Children)
			...
			if (objGear.WeaponID != Guid.Empty.ToString())
			{
                // Remove the Weapon from the TreeView.
                ITreeNode objRemoveNode = displayFactory.CreateTreeNode();
				foreach (ITreeNode objVehicleNode in treVehicles.Nodes[0].Nodes)
				{
					foreach (ITreeNode objWeaponNode in objVehicleNode.Nodes)
					{
						if (objWeaponNode.Tag.ToString() == objGear.WeaponID)
							objRemoveNode = objWeaponNode;
					}
		...
		/// </summary>
		public bool ConfirmDelete(string strMessage, IMessageDisplay messageDisplay)
		{
			if (!_objCharacter.Options.ConfirmDelete)
				return true;
			else
			{
				if (messageDisplay.AskQuestion(strMessage, LanguageManager.Instance.GetString("MessageTitle_Delete")) == false)
					return false;
				else
					return true;
			}
		...
		public void BuildGearTree(Gear objGear, ITreeNode objNode, IContextMenuStrip objMenu)
		{
			foreach (Gear objChild in objGear.Children)
			{
		...
		public void BuildCyberwareTree(Cyberware objCyberware, ITreeNode objParentNode, IContextMenuStrip objMenu, IContextMenuStrip objGearMenu)
		{
		...
		public void CreateArmorTreeNode(Armor objArmor, ITreeView treArmor, IContextMenuStrip cmsArmor, IContextMenuStrip cmsArmorMod, IContextMenuStrip cmsArmorGear)
		{
            ITreeNode objNode = displayFactory.CreateTreeNode();
			...
			foreach (ArmorMod objMod in objArmor.ArmorMods)
			{
				ITreeNode objChild = displayFactory.CreateTreeNode();
			...
			ITreeNode objParent = displayFactory.CreateTreeNode();
            if (objArmor.Location == "")
				objParent = treArmor.Nodes[0];
			else
			{
				foreach (ITreeNode objFind in treArmor.Nodes)
				{
		...
		public void CreateVehicleTreeNode(Vehicle objVehicle, ITreeView treVehicles, IContextMenuStrip cmsVehicle, IContextMenuStrip cmsVehicleLocation, IContextMenuStrip cmsVehicleWeapon, IContextMenuStrip cmsWeaponAccessory, IContextMenuStrip cmsWeaponAccessoryGear, IContextMenuStrip cmsVehicleGear)
		{
			ITreeNode objNode = displayFactory.CreateTreeNode();
			...
			{
				ITreeNode objLocation = displayFactory.CreateTreeNode();
			...
			foreach (VehicleMod objMod in objVehicle.Mods)
			{
				...
				{
					ITreeNode objCyberwareNode = displayFactory.CreateTreeNode();
			...
			foreach (Gear objGear in objVehicle.Gear)
			{
				...
				{
					foreach (ITreeNode objFind in objNode.Nodes)
					{
		...
		public void CreateWeaponTreeNode(Weapon objWeapon, ITreeNode objWeaponsNode, IContextMenuStrip cmsWeapon, IContextMenuStrip cmsWeaponAccessory, IContextMenuStrip cmsWeaponAccessoryGear)
		{
			ITreeNode objNode = displayFactory.CreateTreeNode();
			...
			foreach (WeaponAccessory objAccessory in objWeapon.WeaponAccessories)
			{
				...
				{
					ITreeNode objGearChild = displayFactory.CreateTreeNode();
			...
			{
				foreach (ITreeNode objLocationNode in objWeaponsNode.TreeView.Nodes)
				{
